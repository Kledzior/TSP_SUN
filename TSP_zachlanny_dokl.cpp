...
void TSPzachlanny_dokl::printHamiltonianCycle(const vector<vector<vector<double>>>& graph, const vector<int>& cycle,int choice2)
{
    TSPbrute::printHamiltonianCycle(graph,cycle,choice2);//after thgat i reset values in dlugosc_min,ilosc_cykli,cycle_min
    //  for (int vertex : cycle) {
    //     cout << vertex << " ";
    // }
    // cout << cycle[0] << endl;  // coutuje startowy zeby zrobic zamiast 0-1-2-3 -> 0-1-2-3-0
    // //w funckji w tspbrute ktora liczy dlugosc cyklu mam dodanie na koncu odleglosci miedzy ostatnim a pierwszym stad odleglosc liczy poprawnie
    // cout << "Długość cyklu: " << dlugosc_min << endl;
    // dlugosc_min = numeric_limits<double>::max();
    // ilosc_cykli=0;
    // cycle_min.clear();
}
double TSPzachlanny_dokl::calculateHamiltonianCycleLength(const vector<vector<vector<double>>>& graph, const vector<int>& cycle) 
{
     return TSPbrute::calculateHamiltonianCycleLength(graph, cycle);
}
bool TSPzachlanny_dokl::isEdgeValid(int v, int w, const vector<vector<vector<double>>>& graph, const vector<int>& cycle) 
{
    return TSPbrute::isEdgeValid(v,w, graph, cycle);
}
bool TSPzachlanny_dokl::is_Closest(int v, int w, const vector<vector<vector<double>>>& graph, const vector<int>& cycle)//v - od miasta A , w - do miasta B , z graph , ma byc minimalna odleglosc, cycle - sprawdzamy czy jest zajety
{   
...
void TSPzachlanny_dokl::generateHamiltonianCycles(const vector<vector<vector<double>>>& graph, vector<int>& cycle, int pos,int liczba,int choice2) 
{   
...
void TSPzachlanny_dokl::hamiltonianCycles(const vector<vector<vector<double>>>& graph,int choice2)
{
...
// void TSPzachlanny_dokl::TSP_wynik(const vector<vector<vector<double>>>& graph)
// {
//     nlohmann::json TSP_zachlan_dokl_wynik;
